...
void setup() 
{
  ...
  pixels.setBrightness(_brightness[constrain(EEPROM.read(4), 0, 3)]);
...
void handleButtons()
{
  ...
  // button 2: music reactive mode(long) + random mode/color(double)
  else if (button2())
  {
    bool s = true;
    unsigned long start = millis();
    ...
    if (s)
    {
      ...
      else if (millis() - _lastButtonPress <= DOUBLEPRESSZONE)
      {
        ...
        _currentEffectIndex = (_currentEffectIndex + 1) % _effectsCount;
        _lastShuffle = millis();
